...
경기가 취소되었더라도 실제로 여행을 다녀왔다면 직관로그를 생성할 수 있습니다.

- 경기 상태가 `CANCELLED`여도 기존 직관로그 생성 API를 사용합니다.
- 여행 완료 기준은 기존과 동일하게 한국시간 기준 여행 종료 다음 날입니다.
- 활성 Plan과 경기 정보가 존재해야 하며, 중복 로그 생성은 허용하지 않습니다.
- 기존 여행 일정의 기록 가능한 Entry를 DRAFT 로그에 복사합니다.
- 경기 점수가 없으면 승패 결과를 임의로 생성하지 않습니다.
- 좌석, 동행자, 후기, 사진은 기존 수정·미디어 API를 사용합니다.
- `attendanceLogRequired` 조회에서도 취소 경기라는 이유만으로 제외하지 않습니다.
