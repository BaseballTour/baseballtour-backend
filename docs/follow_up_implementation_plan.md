...
- Firestore에는 독립적으로 수집한 장소명·주소·분류·추천 설명과
  `kakaoPlaceId`만 저장한다.
- 좌표·전화번호·지도 링크는 운영 조회 시 Kakao Local 응답을 일시 결합하며
  Firestore에 영구 복제하지 않는다.
- 배포 전 검사에서 `kakaoPlaceId`가 하나라도 빠지면 실패로 처리한다.

## 2. 선수 추천 장소 사전 보강과 외부 링크

- 영업시간은 현재 저장 대상에서 제외한다. Kakao Local은 영업시간을
  제공하지 않으며, 확인되지 않은 값은 API에서 `MISSING`으로 반환한다.
