...
- 카카오 대중교통·도보 실제시간 비교, 실패 시 거리 기반 fallback
...
- 후보 10·20·30개 및 다중 페이지에서 Kakao Matrix 호출량·실행시간 측정
...
- 추천 후보 준비는 최대 20초이며 초과하면 추천 없이 사용자 선택 장소만으로
  일정 생성을 계속한다.
- 이동시간 Matrix는 카카오 경로 요청을 최대 8개씩 실행한다.
- 개별 경로 요청은 8초, Matrix 전체는 30초의 시간 예산을 사용한다.
- 장소 15개·Anchor 3개 기준 외부 Provider 경로를 342개에서 최대 81개로
  줄이고, 각 Provider 경로에서 대중교통·도보를 병렬 비교한다.
- 전체 시간 예산이 끝나더라도 일정의 뼈대가 정확하도록 도착지·출발지·경기장·숙소가 포함된 Anchor 경로를 먼저 처리한다.
- 실패하거나 시간 예산을 넘긴 경로는 직선거리 기반 도보 예상시간
  `ESTIMATED`를 유지한다.
- Firestore의 UTC 시각은 일정 계산 직전에 `Asia/Seoul`로 변환하여 날짜별
  기본 시작시각 `09:00`을 한국 현지시각으로 적용한다.
- 추천 후보 15개에는 주변 결과가 존재하는 경우 음식점과 카페를 각각 최대
  2개 이상 우선 확보하고 나머지는 거리순으로 채운다.
...
- 실제 Game·Stadium·TourAPI·Kakao Routing·Firestore 통합 테스트
